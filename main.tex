...
% Forces page with only figure to have figure at top
%\makeatletter
%\setlength{\@fptop}{0pt}
%\makeatother
...
\abstract{\unboldmath
From 2012-07-12 through 2012-09-04 (55 days), a magnetotelluric (MT) instrument made continuous 1-second cadence measurements at a site in Middelpos, South Africa. Two methods were used to compute the surface impedance transfer function (TF): (1) 55 TFs were computed and averaged using non-overlapping one-day segments, and (2) one TF was computed using the full 55-day segment. The error bars were calculated using parametric formulas, which assume a known distribution of errors, and a bootstrap method. In addition to coherence, another frequency--domain quality metric, the signal-to-prediction error ($\text{SE}$), was calculated to assess the quality of predictions of the electric field from the computed TFs. The computed TFs have significant off-diagonal components, suggesting a complex ground-conductivity structure. The $\text{SE}$ is greater than unity only in the range of 1-minute to 6-hours for the $E_x$--related terms and 1-minute to 1-hour for $E_y$--related terms. We find differences in the transfer functions computed in the two ways that are inconsistent with the error bars. We also compare the transfer function at Middelpos with that computed at a nearby (${\sim}34$~km) location as part of a different MT campaign. We find significant differences in the transfer functions at the two locations, which suggests significant lateral variations in the ground conductivity structure near Middelpos.
}
...
The electric and fields measured on Earth's surface are not independent of each other and can be related by a transfer function derived from Maxwell's equations (\cite{Vozoff1991, Simpson2005, Utada2026}). The electromagnetic field measured on Earth's surface are primarily due to waves initiated in the ionosphere that penetrate into and reflect off of Earth's conducting surface at various depths \citep{Cagniard1953,Pirjola1982}. The transfer function at Earth's surface is referred to as the surface impedance and relates the horizontal components of the surface $\mathbf{E}$ and $\mathbf{B}$ by a 2x2 matrix with frequency dependent and complex-valued components.

The surface impedance of the Earth near conducting systems, such as power lines and pipelines, is an essential parameter for deriving geomagnetically induced currents (GICs) in the power line from measured or predicted geomagnetic fields. Given a measured or modeled geomagnetic field, the surface impedance can be used to predict the geoelectric field, which can then used to compute GICs in power lines and pipelines (e.g., \cite{Marshall2019,AlvesRibeiro2023}). The surface impedance is also the starting point for estimating the subsurface  conductivity structure because the surface impedance depends on the subsurface conductivity \citep{Simpson2005}.

The surface impedance is typically derived from magnetotelluric (MT) measurements, which combine co-located 3-axis magnetic field measurements with 2-axis horizontal electric field measurements (\cite{Naidu2012}, \cite{Simpson2005}). MT measurements have been made in South Africa by various institutions. The SAMTEX campaign, comprising short-term ($\sim$3 weeks per location) measurements along various transects in South Africa, was conducted by a consortium of geophysicists to characterize the Kaapvaal Craton in the central part of South Africa \citep{Jones2009}. The South African National Space Agency (SANSA) deployed long-term MT measurement stations (operating for several months at each location) at seven locations to provide useful data for GIC estimation by the Space Weather Centre at SANSA \citep{Kotz2015}.
...
Several algorithms have been used to derive the surface impedance transfer function. \cite{Sims1971} introduced regression--based algorithms for computing the components of the impedance tensor. Other algorithms include that in the LEMI software provided with the LEMI-417 instrument produced by the LLC in Lviv, Ukraine \cite{LEMI2025}, the Egbert algorithm (\cite{Egbert1997}, \cite{Egbert2011}), the Jones algorithms (\cite{Jones1989}; \cite{Chave2012}), and various others \cite{Larsen1996}. Recent papers that derived transfer functions from MT measurements include \cite{Jones1989}, \cite{Moorkamp2007}, \cite{Fujii2015}, \cite{Heyns2020}, \cite{Weigel2019}, and \cite{Chen2021}.

In section~\ref{section:Data}, we provide details on the $\mathbf{E}$ and $\mathbf{B}$ time series in the time and frequency domains. There is no single algorithm that is used to compute the surface impedance transfer function. In this work, we use two primary approaches and compare their results. In section~\ref{section:Models_and_Methods}, we describe these two approaches. In section~\ref{section:Results}, we present the results of the transfer function computations at Middelpos. MT measurements and data from a nearby location as part of a MT campaign in 2003 are compred with our results at Middelpos.

%In this paper, we use the regression-based algorithm of \cite{Sims1971} to compute the surface impedance transfer function from MT measurements made at Middelpos, South Africa. We also introduce a new metric for assessing the quality of the computed transfer function, which we call the signal-to-prediction error ($\text{SE}$). This metric quantifies how well the predicted electric field matches the measured electric field.
...
Data were measured with the LEMI-417 instrument, which is designed for long--term recording of MT data; it is comprised of a 3--axis fluxgate magnetometer and a 2--axis electric field antenna. The instrument was installed in an empty field near Middelpos, South Africa (population $\sim$300), located at WGS 84 geographic (latitude, longitude) ($-31.906^\circ$, $20.234^\circ$) and altitude of $1165$~m. The instrument is powered by batteries charged by a solar panel and   is one of eight MT instruments in southern Africa managed by SANSA \citep{Kotz2015}. The timestamps of the measurements are from a GPS chip in the LEMI-417 instrument. Data were manually retrieved during a site visit by SANSA technicians.

The $+x$-axes of both the magnetometer and electric field probe were aligned with geomagnetic North on 2012-07-12 when the magnetic declination at Middelpos was $-23.2541323^\circ$, computed using the definitive IGRF model coefficients for years 2015-2020 \citep{Alken2021}. Figure~\ref{fig:site} is an annotated Google Earth image of the instrument location and electric field probes.
...
Figure~\ref{fig:dft} shows the averaged amplitudes of the Fourier coefficients for each vector component of the time series shown in Figure~\ref{fig:timeseries}. The averaged amplitudes are frequency--domain band averages of the discrete discrete Fourier transform amplitudes. The values of the band centers, marked with filled circles, the number of values in each band, and the band boundaries are given in Table~\ref{table:evalfreqs55d} of Appendix~\ref{appendix:tables}. The vertical bars correspond to 95\% confidence intervals; the length of the confidence interval for the magnitude of a Fourier coefficient, $\Delta|\widetilde{A}|$, calculated using propagation of errors on $|\widetilde{A}| = \sqrt{A_\text{r}^2 + A_\text{i}^2}$, is
...
\noindent
where $\Delta A_\text{r}$ and $\Delta A_\text{i}$ are the lengths of the 95\% confidence intervals for the real, $A_\text{r}$, and imaginary, $A_\text{i}$, parts of $\widetilde{A}$, respectively; $\widetilde{A}$ is one of the $\widetilde{B}_x$, $\widetilde{B}_y$, $\widetilde{E}_x$, and $\widetilde{E}_y$, which are frequency domain averages shown in Figure~\ref{fig:dft}. The lengths of the symmetric 95\% confidence intervals for the averages $A_\text{r}$ and $A_\text{i}$ were determined using $\pm 2 (s/\sqrt{n})t^{-1}_{n-1}(0.025)$, where $n$ is the number of values used to calculate the average, $t^{-1}_{n-1}(0.025)$ is the inverse $t$-distribution evaluated at $0.025$, and $s$ is the standard deviation of the sample.

\begin{figure}
  \includegraphics[width=\textwidth]{figures/tsplot-original-Middelpos-tf1.pdf}
  \caption{Measurements at Middelpos used for analysis after mean subtraction and despiking.}
  \label{fig:timeseries}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{figures/dftplot-original-averaged-magnitudes-Middelpos-tf1.pdf}
  \caption{Magnitudes of discrete Fourier transform coefficients for measurements in Fig~\ref{fig:timeseries}. The vertical bars correspond to 95\% confidence intervals and a filled circle indicates the computed lower bound is below zero.}
  \label{fig:dft}
\end{figure}
...
\noindent using a method described by \cite{Sims1971}: for each vector component of $\mathbf{E}$, $k=x\mbox{ or }y$, a four parameter over--determined linear regression is performed using
...
\noindent with values of $f$ in a band associated with $f_e$. The number of parameters is four because the two regression parameters $\widetilde{Z}_{kx}$ and $\widetilde{Z}_{ky}$ in Equation~\ref{eqn:single} are complex--valued; the actual regression is performed using only real numbers \citep{Egbert1986}. The values of $f$ are those associated with Fourier coefficients of $E_k(t)$ and $B_k(t)$, $\widetilde{E}_k(f)$ and $\widetilde{B}_k(f)$. This regression method corresponds to minimizing the least-squared error between the predicted and measured $\widetilde{E}_k(f)$ values in each band.
...
\noindent
where $\Delta \widetilde{Z}_{ij}$ is the 95\% confidence interval length for $\widetilde{Z}_{ij}$, $|\boldsymbol{\cdotp}|$ is the complex magnitude, and $\text{Re}(\boldsymbol{\cdotp})$ and $\text{Im}(\boldsymbol{\cdotp})$ are the real and imaginary parts of the argument, respectively.
...
95\% confidence intervals were computed using bootstrap resampling: for each $f_e$ band, the $N_e$ pairs of $\widetilde{Z}_{kx}$ and $\widetilde{Z}_{ky}$ were randomly selected with replacement, and the sums in the $\text{Coh}$ formula were computed using these randomly selected pairs to compute a bootstrap coherence $\text{Coh}^*$. This process was repeated $1,000$ times, and the results were sorted in ascending order; the maximum $\text{SE}$ error bar value is the $975$th value in the sorted $\text{Coh}^*$ list, and the minimum is the $25$th. No confidence intervals were computed if $N_e\le 10$.

Traditionally, the primary quality metric used for evaluating the transfer function is $\text{Coh}^2_{E_xB_y}$ and $\text{Coh}^2_{E_yB_x}$. A limitation of this metric is that it does not directly quantify how well the Fourier coefficients of the predicted electric field, $\widetilde{E}^p_k$, match those of the measured electric field, $\widetilde{E}_k$. It quantifies only the coherence between the measured quantities. As a result, we have also computed the coherence between the Fourier coefficients of the measured and predicted electric field, $\text{Coh}^2_{E_kE^\text{p}_k}$ ($k=x\mbox{ or }y$), as it is a more direct measure of the ability of the transfer function to reproduce the signal.
...
\noindent where the summation frequencies and $f_e$ values are the same as those used for computing $\text{Coh}$.
...
In Figure~\ref{fig:timeseries-KAP103}, the 26--day magnetic and electric time series are shown along; the magnitude of their Fourier coefficients is shown in Figure~\ref{fig:dft-KAP103}. The measurement interval included the large geomagnetic storm of November 20--22, 2003, that followed the October 31st ``Halloween" storm \citep{Gopalswamy2005}.
...
\begin{figure}[!ht]
  \includegraphics[width=\textwidth]{figures/tsplot-original-KAP103-tf1.pdf}
  \caption{Measurements from KAP-103 used for analysis after mean subtraction and despiking.}
  \label{fig:timeseries-KAP103}
\end{figure}

\begin{figure}[!hb]
  \includegraphics[width=\textwidth]{figures/dftplot-original-averaged-magnitudes-KAP103-tf1.pdf}
  \caption{Magnitudes of discrete Fourier transform coefficients for measurements in ~\ref{fig:timeseries-KAP103}. The vertical bars correspond to 95\% confidence intervals.}
  \label{fig:dft-KAP103}
\end{figure}
